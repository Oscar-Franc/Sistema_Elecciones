...
try {
    ...
    // Consultas para obtener los datos de votos de la organización 3 (id_organizacion = 3)

    // Votos de alumnos de la organización 3 (usando la relación con Carrera)
    $sql_alumnos = "SELECT COUNT(*) AS total_alumnos, SUM(a.voto_realizado) AS votos_realizados
                    FROM Alumno a
                    JOIN Persona p ON a.id_persona = p.id_persona
                    JOIN Carrera c ON a.id_carrera = c.id_carrera
                    WHERE c.id_Organizacion = 3";
    $alumnos_result = $pdo->query($sql_alumnos);
    $alumnos = $alumnos_result->fetch();

    // Votos de profesores de la organización 3
    ...
    // Consultar el total de votos realizados por hombres y mujeres en la organización 3
    $sql_sexo = "SELECT sexo, SUM(voto_realizado) AS votos_realizados
                 FROM Persona p
                 JOIN (
                    SELECT id_persona, voto_realizado FROM Alumno WHERE id_carrera IN (SELECT id_carrera FROM Carrera WHERE id_Organizacion = 3)
                    UNION
                    SELECT id_persona, voto_realizado FROM Profesor WHERE id_organizacion = 3
                    UNION
                    SELECT id_persona, voto_realizado FROM Administrativo WHERE id_organizacion = 3
...
<h1>Estadísticas de votación a nivel Teotihuacan</h1>
