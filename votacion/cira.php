...
try {
    ...
    // Consultas para obtener los datos de votos de la organización 2 (id_organizacion = 2)

    // Votos de alumnos de la organización 2 (usando la relación con Carrera)
    ...
    // Votos de profesores de la organización 2
    ...
    // Consultar el total de votos realizados por hombres y mujeres en la organización 2
    $sql_sexo = "SELECT sexo, SUM(voto_realizado) AS votos_realizados
                 FROM Persona p
                 JOIN (
                    SELECT id_persona, voto_realizado FROM Alumno WHERE id_carrera IN (SELECT id_carrera FROM Carrera WHERE id_Organizacion = 2)
                    UNION
                    SELECT id_persona, voto_realizado FROM Profesor WHERE id_organizacion = 2
                    UNION
                    SELECT id_persona, voto_realizado FROM Administrativo WHERE id_organizacion = 2
...
<h1>Estadísticas de votación a nivel CIRA</h1>
