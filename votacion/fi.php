...
try {
    ...
    // Consultas para obtener los datos de votos de la organización 1 (id_organizacion = 1)

    // Votos de alumnos de la organización 1 (usando la relación con Carrera)
    $sql_alumnos = "SELECT COUNT(*) AS total_alumnos, SUM(a.voto_realizado) AS votos_realizados
                    FROM Alumno a
                    JOIN Persona p ON a.id_persona = p.id_persona
                    JOIN Carrera c ON a.id_carrera = c.id_carrera
                    WHERE c.id_Organizacion = 1";
    $alumnos_result = $pdo->query($sql_alumnos);
    $alumnos = $alumnos_result->fetch();

    // Votos de profesores de la organización 1
    ...
    // Consultar el total de votos realizados por hombres y mujeres en la organización 1
    $sql_sexo = "SELECT sexo, SUM(voto_realizado) AS votos_realizados
                 FROM Persona p
                 JOIN (
                    SELECT id_persona, voto_realizado FROM Alumno WHERE id_carrera IN (SELECT id_carrera FROM Carrera WHERE id_Organizacion = 1)
                    UNION
                    SELECT id_persona, voto_realizado FROM Profesor WHERE id_organizacion = 1
                    UNION
                    SELECT id_persona, voto_realizado FROM Administrativo WHERE id_organizacion = 1
    ...
    // Total de votos realizados (hombres + mujeres)
    $total_votos_realizados = $votos_hombres + $votos_mujeres;

    // Calcular los porcentajes de votación por sexo
    $porcentaje_hombres = $total_votos_realizados > 0 ? ($votos_hombres / $total_votos_realizados) * 100 : 0;
    $porcentaje_mujeres = $total_votos_realizados > 0 ? ($votos_mujeres / $total_votos_realizados) * 100 : 0;
...
<h1>Estadísticas de votación a nivel Facultad de Ingenieria</h1>
